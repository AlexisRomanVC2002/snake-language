...
const IDENTIFICADOR = /^\$[A-Za-z][A-Za-z_]*$|^\$[A-Za-z][A-Za-z_]*$/; //^\$[A-Za-z_$][A-Za-z_$]*$/; ///^[A-Za-z][A-Za-z_]*$/;
...
// cicle ---> cicle($numero < 10){}
// for ---> for($numero){} ---> se le pasa el numero de veces que se va a repetir el ciclo.
...
export function Scanner(entrada_codigo) {
    ...
    for (let i = 0; i < lineas_limpias.length; i++) {
        ...
        if (palabra != "") {
            ...
            let error = {
                linea : i,
                contenido: lineas_codigo[i - 1].trim()
            }
        ...
        for (let index_caracter = 0; index_caracter < linea.length; index_caracter++) {
            ...
            // Identificamos si la palabra creada es un numero. ---> Nota Resolver que si no hay un ; no identifica numero
            ...
            // Se verifica si la palabra es un operador.
            else if (OPERADORES.includes(palabra)  && (!OPERADORES.includes(linea[index_caracter + 1]))) {
                ...
                switch (palabra) {
                    ...
                    case "%" : token = new Token(TIPOS_TOKEN.APERTURA_BLOQUE, palabra, i + 1);
                    break;

                    case "%%" : token = new Token(TIPOS_TOKEN.CERRADURA_BLOQUE, palabra, i + 1);
                    break;
            ...
            else if (palabra.match(IDENTIFICADOR) && (OPERADORES.includes(linea[index_caracter + 1]) || DELIMITADORES.includes(linea[index_caracter + 1]) || !linea[index_caracter + 1] )) {
    ...
    if (palabra != "") {
        ...
        let error = {
            linea : lineas_codigo.length,
            contenido: lineas_codigo[linea - 1].trim()
        }
